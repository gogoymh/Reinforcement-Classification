...
class ReinforceNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out1 = self.bn1(out1) + cum_feat
        cum_feat = cum_feat + out1
        
        out2 = self.relu(out1)
        out2 = self.conv2(out2)
        out2 = self.bn2(out2) + (cum_feat/2)
        cum_feat = cum_feat + out2
        
        out3 = self.relu(out2)
        out3 = self.conv3(out3)
        out3 = self.bn3(out3) + (cum_feat/3)
        cum_feat = cum_feat + out3
        
        out4 = self.relu(out3)
        out4 = self.conv4(out4)
        out4 = self.bn4(out4) + (cum_feat/4)
        cum_feat = cum_feat + out4
        
        out5 = self.relu(out4)
        out5 = self.conv5(out5)
        out5 = self.bn5(out5) + (cum_feat/5)
        cum_feat = cum_feat + out5
        
        out6 = self.relu(out5)
        out6 = self.conv6(out6)
        out6 = self.bn6(out6) + (cum_feat/6)
        cum_feat = cum_feat + out6
        
        out7 = self.relu(out6)
        out7 = self.conv7(out7)
        out7 = self.bn7(out7) + (cum_feat/7)
        cum_feat = cum_feat + out7
        
        out8 = self.relu(out7)
        out8 = self.conv8(out8)
        out8 = self.bn8(out8) + (cum_feat/8)
        cum_feat = cum_feat + out8
        
        out9 = self.relu(out8)
        out9 = self.conv9(out9)
        out9 = self.bn9(out9) + (cum_feat/9)
        cum_feat = cum_feat + out9
        
        out10 = self.relu(out9)
        out10 = self.conv10(out10)
        out10 = self.bn10(out10) + (cum_feat/10)
        cum_feat = cum_feat + out10
        
        out11 = self.relu(out10)
        out11 = self.conv11(out11)
        out11 = self.bn11(out11) + (cum_feat/11)
        cum_feat = cum_feat + out11
        
        out12 = self.relu(out11)
        out12 = self.conv12(out12)
        out12 = self.bn12(out12) + (cum_feat/12)
        cum_feat = cum_feat + out12
        
        out13 = self.relu(out12)
        out13 = self.conv13(out13)
        out13 = self.bn13(out13) + (cum_feat/13)
        cum_feat = cum_feat + out13
        
        out14 = self.relu(out13)
        out14 = self.conv14(out14)
        out14 = self.bn14(out14) + (cum_feat/14)
        cum_feat = cum_feat + out14
        
        out15 = self.relu(out14)
        out15 = self.conv15(out15)
        out15 = self.bn15(out15) + (cum_feat/15)
        cum_feat = cum_feat + out15
        
        out16 = self.relu(out15)
        out16 = self.conv16(out16)
        out16 = self.bn16(out16) + (cum_feat/16)
...
torch.save({'epoch': epoch,
            ...
            'loss': losses[epoch].item()}, "C:/유민형/개인 연구/Reinforcement Classification/results/RLnet16_d2.pkl")

#"/DATA/ymh/rlcl/results/RLnet16_d2.pkl"
