...
class ReinforceNet(nn.Module):
    ...
    def make_conv_layer(self, channel):
        ...
        layer = [nn.Conv2d(channel,channel,3,1,1,bias=False), nn.Dropout2d(0.1)]
    ...
    def forward(self, x, train=True):
        ...
        if train:
            ...
            image2 = out1[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image3 = out2[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image4 = out3[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image5 = out4[1].clone().detach().cpu().numpy().transpose(1,2,0)
            
            image6 = out5[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image7 = out6[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image8 = out7[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image9 = out8[1].clone().detach().cpu().numpy().transpose(1,2,0)
            
            image10 = out9[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image11 = out10[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image12 = out11[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image13 = out12[1].clone().detach().cpu().numpy().transpose(1,2,0)
            
            image14 = out13[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image15 = out14[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image16 = out15[1].clone().detach().cpu().numpy().transpose(1,2,0)
            image17 = out16[1].clone().detach().cpu().numpy().transpose(1,2,0)
